...
async def add_class_and_id(id, class_=None):
    ...
    if class_ == None:
        ...
        if cursor.fetchone() is None:
            cursor.execute(
                "INSERT INTO memory_class (id, class) VALUES(?, ?)", [id, ""]
            )
            db.commit()
    else:
        cursor.execute("SELECT * FROM memory_class WHERE id=?", [id])
        if cursor.fetchone() is None:
            cursor.execute(
                "INSERT INTO memory_class (id, class) VALUES(?, ?)", [id, class_]
            )
            db.commit()
        else:
            cursor.execute(
                f"Update memory_class SET class = '{class_}' WHERE id = '{id}'"
            )
            db.commit()
    cursor.close()
    db.close()
...
async def check_class_id(id):
    ...
    if (
        list(cursor.execute("SELECT class FROM memory_class WHERE id=?", [id]))[0][0]
        == ""
    ):
        cursor.close()
        db.close()
...
async def get_class_id(id):
    ...
    class_ = list(cursor.execute("SELECT class FROM memory_class WHERE id=?", [id]))[0][
        0
    ]
    cursor.close()
    db.close()
