...
kb_get_schedule.row()
kb_get_schedule.add(
    Text("Настройки", {"cmd": "settings"}), color=KeyboardButtonColor.SECONDARY
)
kb_get_schedule.row()
kb_get_schedule.add(
    OpenLink(
...
kb_subscribe_to_newsletter.add(
    Text("Подписаться на рассылку", {"cmd": "subscribe_on_newsletter"}),
    color=KeyboardButtonColor.POSITIVE,
)
kb_subscribe_to_newsletter.row()
kb_subscribe_to_newsletter.add(
    Text("Назад", {"cmd": "back1"}), color=KeyboardButtonColor.PRIMARY
)
kb_unsubscribe_from_mailing_list.add(
